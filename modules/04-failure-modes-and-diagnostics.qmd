...
(JO: Where does the sequential fitting fit into this fitting diagnostic pipeline?)
...
- Degeneracy is a classic **model-specification** error, widely discussed in canonical papers.(JO: if we're using footnotes, include references to these papers here.) 
(JO: more importantly, we need to say what degeneracy is, to provide at least some intuition)

- Can be scale dependent: At the **simplified** n = 1,000 scale, `odegree(0:2)` fits fine. 

- But `odegree(0:3)` produced the same error at the n=1,000 scale. 

::: {.accent}
Fix: no control setting rescues degeneracy: the **model specification** needs to be changed. (JO: I thought the sequential approach helped with this? Or was that with the non-convergence? You see, even I need to understand what degeneracy is.)
:::
...
settling around a stable mean) 
 ...
 - The edge density plots may be centered far from 0, indicating divergence between the targets and fitted values. (JO: where is the target value for the edges in this figure?)
