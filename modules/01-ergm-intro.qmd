...
2. **Summarize** it numerically: these become our **targets** (JO: can we say "these network summary statistics become our calibration targets"?)
(ASK: Not really sure, the calibration targets usually are downstream from the network?)
3. **Model** it: null (random mixing) vs. assortative mixing (representing a simple mechanism that drives adolescent friendships) (JO: This is somewhat opaque. Why assortative here?. ASK: see addition in parentheses.)
4. **Simulate**: does the model reproduce the underlying empirical structures? (JO: "in the form of our network summary statistics?")
...
(JO: explain plogis?)
::: notes
(JO: talk about the use of log-odds if we use it throughout this and later modules)
