...
## How this workshop came about {.smallish}

::: footer
&nbsp;
:::

- This is a research problem on fitting an exponential random graph model (ERGM) to a directed **32,000-node** syringe-sharing
  network (HepCEP). 
  
- We first modeled this using the `ergm` v3.x, part of the `statnet` suite in R.^[Boodram et al. (2022), *PLoS ONE* 17(1): e0248850. doi:10.1371/journal.pone.0248850]

- As the synthetic population evolved to incorporate more realistic demographic structures, we needed to refit the ERGM. 

- But, the ERGM no longer fit "out of the box".

---

## How this workshop came about (cont'd) {.smallish}

::: footer
&nbsp;
:::

- Refitting started with a long conversation with the  **statnet_help community** in 2024.

- Following this, we worked through a chain of modeling steps and tuning of fitting parameters, often starting with some educated guesswork. 

- Once we got the ERGM fitting sequence to work, we wanted to share the decisions we worked through as a pipeline with the ABM community interested in incorporating empirical networks.

---

## Why we Developed this Workshop {.smallish}

::: footer
&nbsp;
:::

- The **ABM community** increasingly wants networked populations — but the practical
  ERGM-fitting know-how is **scattered and largely undocumented**.
- This workshop packages that pipeline as a **reproducible, hands-on toolkit**:
  aggregate summaries → convergent ERGM → simulated networks → ABM inputs.
- The emphasis is **practical judgment**, including diagnosis, tuning, and checking fits in practice. 
- The workshop example includes a 1000-node network extracted from our "research problem," to enable us to experiment with the steps in real time. 
- The full pipeline will be made available in a GitHub repository once the paper is submitted for review.

![](img/boodram-plos-2022.png){width="34%" fig-align="right"}

---

## Module 1: Introducting the Core Concepts
...
- The same **micro → macro** process that **ABMs** are useful for. 
- That is, localized rules produce emergent structure. 
- Here, this **micro → macro** pipeline is followed for modeling network structure.
...
2. **Summarize** it numerically: these network **summary statistics** become our **targets** for evaluating the quality of the fitted ERGM (akin to calibration in ABMs).
3. **Model** it: null (random mixing) vs. **assortative** mixing, incorporating a known mechanism ("homophily") of friendship formation.
4. **Simulate**: do networks from the model reproduce the empirical structure — i.e., our **target statistics**?
...
- The `edges` coefficient is a **log-odds**.
- We convert it to a probability using `plogis()`, representing the "baseline" chance any two students are friends.
- The `edges` term models this baseline probability, akin to the **intercept** in regression modeling.

::: notes
Log-odds are the currency throughout: ERGM coefficients are on the log-odds scale (as in
logistic regression), and `plogis()` (inverse-logit) maps them to probabilities. Here the
`edges` coef is about −6.99 → probability ≈ 0.0009: a random pair almost never shares a tie.
:::
...
- These `nodematch` terms allow us to estimate the probability that persons within the same grade, or of the same sex or race, are more likely to be friends.
- This is a **dyad-independent** model, a tie's odds depend only on the two nodes' attributes. 
- Such models can be fit using standard logistic regression, and fit fast. 
- Positive coefficients → ties are more likely **within** that attribute (homophily).



::: notes
The point to foreshadow: dyad-independent terms fit by plain logistic regression — no need to
search the space of possible networks. That changes with dyad-*dependent* terms (triangles /
GWESP, coming up), where whether one tie exists depends on other ties, so `ergm` must explore
network configurations by MCMC — slower, and prone to degeneracy.
:::
...
share that attribute. Log-likelihood (how well the model explains the observed ties) also
jumps (−7790 to −6528): a better fit.
:::
...
- "Better" is measured here in terms of **log-likelihood**. 
- Higher **log-likelihood** means the observed ties are **more probable** under the model.
- But does the better fitting model *reproduce* the network structure we care about?
- The test: simulate many networks, compare to the target network statistics. 

::: notes
Intuition: a converged model matches the statistics you fit it to (in expectation), but it's
free to differ on anything you didn't constrain. The assortative model matches mixing, yet its
simulated networks miss the degree distribution (next slides) — degree was never a target. So
convergence guarantees the *included* terms; alignment on *unmodeled* structure is a separate
check. We pick a structural property and see whether the data sits inside the simulated spread.
:::
...
- So each additional shared friend should add **less** to the tie's log-odds than the one before (i.e., **diminishing returns**).
- A `triangle` parameter instead adds a *constant* amount per shared partner (linear, unbounded), which drives **model degeneracy** (discussed later).
- Hence we specify a parameter with these diminishing returns.
...
- Dyad-dependent, i.e., the probability of two nodes being connected is dependent on other ties in the network.
- This one is fit by **MCMC**, unlike the models above, which used MLE.
...
`R/precompute-intro-fit.R`; the coefficients shown are read from `intro-fits.rds`.
:::
...
## Simulate from the fitted model {.smaller}

A single network drawn from the assortative fit, beside the observed network:

![](precomputed/obs-vs-sim.png){width="90%" fig-align="center"}

- A simulated network is a **draw from the model** (not a copy of the data).
- It reproduces what the model includes (i.e., assortative mixing). 
