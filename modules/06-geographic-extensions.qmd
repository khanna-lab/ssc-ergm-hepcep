...
::: {.gap}
Two things to note when you install it yourself:
:::

- The package will need to be **reinstalled whenever you upgrade `ergm`**, since it links to it.
- Under `renv`, it must be installed into the **project** library. A system-library
  copy is invisible to the project and `library()` will fail.
...
That predicts `1.n = 1`, `1.f = 1`, `2.n = 0`, and `2.f = 1`.
...
Drop category 1's cutoff to 100 m and the 0.73 km tie should reclassify as far:
...
otherwise nearly impossible to debug inside C.
:::
...
On our n = 1,000 network, a **random** placement of ~900 ties lands almost entirely
in the far cells:
...
::: {.accent}
Geographic proximity is the dominant structure in this network. The term has to
create nearly every near tie from scratch.
:::
...
## The sibling term `dist()` {.smallish}

The same package ships a simpler, older term that bands every tie by distance,
with no category:

```r
summary(g ~ dist(1:4))          # band indices, not distances
#> dist1 dist2 dist3 dist4
#>     0     1     2     0
```

The four default bands follow the field's conventional cutoffs:
...
- `breaks` replaces the cutoffs, so `dist(1:3, breaks = c(1, 3))` gives three bands.
- `dnf` adds the piece `dist` still lacks, one threshold per category.

::: notes
`dist` came first and answered "how far are ties, overall". `dnf` answers "how far
are ties, given their source", which is the question our survey summaries can
actually speak to. Both terms take custom cut points now, so the live distinction
is the per-category stratification. Warn that the first argument is band indices,
since asking for `dist(1:7)` with four bands is an error.
:::
...
`dnf` rests on five assumptions, and version 4 checks all but one.
...
| `by` yields integers `1, 2, ..., n` | Codes index the threshold vector | Error naming the observed range |
| One threshold per category | `thresholds` sets n | Error, from the same check |
...
::: notes
Version 3.1.1 used the category codes as raw array indices, so a factor, a 0/1
coding, or categories `c(1, 3)` each produced plausible numbers that were wrong.
Version 4 rejects all three by name. Recoding to consecutive integers from 1 is
still the preprocessing step, but you now find out immediately.
:::
...
4. **Verify** on a network small enough to check mentally.
...
- Compiled terms get **verified on tiny networks**, before they get trusted.
...
1. Install from `hepcep/ergm.userterms.hepcep`, then rebuild the three-node Hyde
   Park network above.
2. Run `summary(g ~ dnf(...))` at all four values of `base`. Confirm the four cells
   sum to the edge count.
3. Set `thresholds = c(0.1, 2)`. Predict which cells move **before** you run it.
4. On the n = 1,000 network, swap `nodematch("chicago")` for
   `dnf(by = "chicago", thresholds = c(2, 2))` and check the statistic count.
5. Refit the mixing block with the new targets. Does it still converge?
6. Simulate 100 networks (Module 5) and check the `dnf` cells against their targets.

::: notes
Steps 1-3 are the core and should fit comfortably. Steps 4-6 depend on the package
being pre-installed in the image, so treat them as the stretch goal and have a
pre-baked fit ready if installation eats the clock.
:::
