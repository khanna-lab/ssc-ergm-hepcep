...
::: {.punch}
Same logic as `levels2 = -1` in Module 2.
:::

::: {.gap}
`base` picks *which* cell is omitted.
:::
...
# What's involved in creating a custom `ergm` term?
...
# Custom `ergm` terms: trust but verify
...
# Applying the custom `ergm` term to our n = 1,000 network
...
One line changes in the mixing block of `exercises/03-sequential.qmd`:
...
length(summary(f_mix))    # 25 statistics -- was 23 with nodematch("chicago")
length(ts_mix)            # 25 targets -- the two must match
```
...
- `k` breaks give `k + 1` bands, so the three defaults yield four. Every dyad
  falls in exactly one band, so `edges + dist(1:4)` is overdetermined. We drop a band
...
# What did we learn about custom `ergm` terms?
